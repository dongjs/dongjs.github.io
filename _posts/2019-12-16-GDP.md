...
We study the question in the title in the context of *differential privacy*, the most successful quantitative measure of privacy to this date. But there are still many versions: [$$(\varepsilon,\delta)$$](https://www.cis.upenn.edu/~aaroth/Papers/privacybook.pdf), [Renyi](https://arxiv.org/abs/1702.07476), [concentrated](https://arxiv.org/abs/1605.02065) and [truncated concentrated](https://dl.acm.org/citation.cfm?id=3188946)... Which one should we use? Maybe it's time to take a step back and see what is shared by these notions.

A favorable approach to provide rigorous security/privacy guarantee is by reasoning about the potential adversary:

1. What kind of attack does he want?
2. How would he achieve it?
3. Design algorithms so that he can’t do that.

Differential privacy (DP) follows the same agenda, with the following attack in mind: the adversary is given the (randomized) algorithm $$M$$ and its output $$y$$, and attempts to infer whether the input database is $$D$$ or $$D'$$. This is often called "differential attack" because his goal is to differentiate two cases.

Next question: how would he achieve differential attack? A valuable feature of DP is that it provides worst case guarantee. So we would like to think about the most powerful adversary. It turns out the most powerful algorithm to identify the input is based on a popular notion in DP community: privacy loss.
...
It is helpful to consider the most private mechanism $$M_0$$ that ignores input and always output a certain distribution. Among all mechanisms, the adversary has the least differential power working against this one --- he can do no better than random guessing, which corresponds to the line type I error $$+$$ type II error $$=1$$. In other words, $$\mathrm{Diff}_{M_ {0}}(x) = 1-x$$. Of course, the graph (dashed line) is above that of $$f_{\varepsilon,\delta}$$.
...
In randomized response, each individual holds one bit of sensitive information $$b\in\{0,1\}$$ and reports the true value $$b$$ with probability $$p$$ and its negation $$\neg b$$ with probability $$1-p$$. The randomization procedure $$M:\{0,1\}\to\{0,1\}$$ has the property that $$M(1)\sim\mathrm{Bern}(p), M(0)\sim\mathrm{Bern}(1-p)$$. When $$p$$ is close to 1, $$M(b)=b$$ with high probability. When $$p=1/2$$, $$M(0)$$ and $$M(1)$$ has exactly the same distribution. Easiest case for the adversary is when two bits are different, i.e.
...
When we do Algorithm 1 (aka likelihood ratio test) for the adversary, we can plot the differential power $$\mathrm{Diff}_M = T_{\mathrm{Bern}(p), \mathrm{Bern}(1-p)}$$ as the blue line. We see
...
From adversary's perspective, he has to differentiate two shifted Laplace distributions. Easiest case occurs when $$q(D)$$ and $$q(D')$$ differ by 1. That is, differential power against Laplace mechanism is

$$\mathrm{Diff}_M = \inf_{D,D'} T_{M(D), M(D')} = T_{\mathrm{Lap}(0,\tfrac{1}{\varepsilon}), \mathrm{Lap}(1,\tfrac{1}{\varepsilon})}.$$

Likelihood ratio test for two shifted Laplace is also easy. It turns out that in this case $$\mathrm{Diff}_M$$ mostly agrees with $$f_{\varepsilon,0}$$ except in the interval $$[\frac{1}{2}\mathrm{e}^{-\varepsilon},\frac{1}{2}]$$. In that interval it is a reciprocal function $$y=\mathrm{e}^{-1}/4x$$. In summary, equality almost holds.
...
For a numerical query $$q$$, Gaussian mechanism returns $$N\big(q(D), \sigma^2\big)$$. It was known that if $$\sigma=\frac{1}{\varepsilon}\sqrt{2\log(1.25\delta^{-1})}$$ then Gaussian mechanism is $$(\varepsilon,\delta)$$-DP. Better formula exists but the improvement is hardly noticeable in our context.

Similar to Laplace mechanism, we have
$$\mathrm{Diff}_M = T_{N(0,\sigma^2), N(1,\sigma^2)}.$$
For two pairs of $$(\varepsilon,\delta)$$, namely $$(1,10^{-3})$$ and $$(4,10^{-3})$$, we use the formula above to determine the corresponding $$\sigma$$ and plot both the $$(\varepsilon,\delta)$$ design goal (dashed red) and the true differential power (blue). Dashed black line corresponds to perfect privacy. We see the red and blue curves are far from equal in both cases.
<p align="center">
...
2. In terms of differential power, compostion corresponds to product distribution.
...
## Composition of Laplace Mechanism
Admitting the two principles, when $$M$$ is a composition of Laplace mechanisms, we have
