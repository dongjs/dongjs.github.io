...
So even though Alice and Bob are running exactly the same algorithm, they may end up with arbitrarily different privacy conclusions using Theorem 0. That is, Bob can severely underestimate the privacy that he already has. In that unfortunate case, he either reports very weak privacy guarantee, or increases the noise level and ruins the accuracy.
...
## Root of All Evil
In analogy to sensitivity, we define a quantity called range to relieve us from the trouble.
\begin{align}
\mathrm{sens}(q)&:= \sup_{x\sim x'} \max_{j\in J} |q(x',y)-q(x,y)|\\\\\\
\mathrm{range}(q)&:= \sup_{x\sim x'} (\max_y -  \min_y) [q(x',y)-q(x,y)].
\end{align}

This quantity has the nice property that if two scores incur the same algorithm then they have the same range. More precisely,

**Proposition** If $$\mathrm{Exp}_{q,\varepsilon}=\mathrm{Exp}_{q',\varepsilon}$$ then $$\mathrm{range}(q) = \mathrm{range}(q')$$.

Range also leads to a better privacy theorem than sensitivity.

**Theorem 1** [[DDR '19]](https://arxiv.org/abs/1909.13830) $$\mathrm{Exp}_{q,\varepsilon}$$ is $$\mathrm{range}(q)\cdot\varepsilon$$-DP.

It is better than Theorem 0 because

1. $$\mathrm{range}(q)\leqslant 2\mathrm{sens}(q)$$.
2. $$\mathrm{range}(q) = \mathrm{sens}(q)$$ if $$q$$ is monotone, i.e. $$q(x,y)\leqslant q(x',y)$$ if $$x\subseteq x'$$[^1].

[^1]: This relies on the neighboring model being add/remove. Notation in the post is for replace model for simplicity.

In particular, counting queries are monotone, with $$\mathrm{range} = \mathrm{sens} = 1$$.
...
These asymptotic results make it clear that fine-grained analysis can buy a factor of 4 in terms of privacy. If we want to reach the same level of final privacy, then the improved (in fact, optimal) analysis allows us to answer 16$$\times$$ more queries. This can be crucial for DP at LinkedIn: advertizers are probably ok with 16 queries per day, but definitely not with 1 query per day.

## Conclusion

1. For exponential mechanisms, forget sensitivity. Use range.
2. You can buy a factor of 4 in privacy parameters from classcial analysis in composition of exponential mechanisms.
